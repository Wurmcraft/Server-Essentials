...
public class TPACommand {
  ...
  public void tpaPlayer(ICommandSender sender, EntityPlayer otherPlayer) {
    if (sender.getCommandSenderEntity() instanceof EntityPlayer) {
      ...
      StoredPlayer otherPlayerData = PlayerUtils.get(otherPlayer);
      if (!otherPlayerData.server.tpLock) {
        requestingTPA.add(new Object[]{
            (System.currentTimeMillis() + (GeneralModule.config.tpaRequestTimeout
                * 1000)), otherPlayer, player});
        ...
        ChatHelper.sendMessage(sender,
            PlayerUtils.getLanguage(sender).GENERAL_TPLOCK_NOTIFY
                .replaceAll("%PLAYER%", otherPlayer.getDisplayNameString()));
      }
    }
  }
