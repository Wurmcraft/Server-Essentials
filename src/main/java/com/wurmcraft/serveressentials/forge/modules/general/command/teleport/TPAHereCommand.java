...
public class TPAHereCommand {
  ...
  public void tpaPlayer(ICommandSender sender, EntityPlayer otherPlayer) {
    if (sender.getCommandSenderEntity() instanceof EntityPlayer) {
      ...
      requestingTPA.add(new Object[]{(System.currentTimeMillis() + (GeneralModule.config.tpaRequestTimeout * 1000)), player, otherPlayer});
      ChatHelper.sendMessage(sender, PlayerUtils.getLanguage(sender).GENERAL_TPAHERE_SENT
          .replaceAll("%PLAYER%", otherPlayer.getDisplayNameString()));
      ChatHelper.sendMessage(otherPlayer,
          PlayerUtils.getLanguage(otherPlayer).GENERAL_TPAHERE_REQUEST
