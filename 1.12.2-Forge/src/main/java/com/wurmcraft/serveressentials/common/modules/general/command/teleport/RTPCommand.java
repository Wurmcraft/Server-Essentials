...
public class RTPCommand {
    ...
    public void rtp(ServerPlayer player) {
        ...
        if (spawn == null)
            spawn = new Location(world.getSpawnPoint().getX(), world.getSpawnPoint().getY(), world.getSpawnPoint().getZ(), 0, 0, 0);
        top:
        while (true) {
            ...
            if (isSafeLocation(world, (int) x, y, (int) z)) {
                ...
                // Valid Location
                boolean valid = false;
                for (int validDim : ((ConfigGeneral) SECore.moduleConfigs.get("GENERAL")).rtpDimensionWhitelist)
                ...
                if (TeleportUtils.teleportTo((EntityPlayerMP) player.player, player.local, new Location(x, y, z, player.player.dimension, player.player.rotationPitch, player.player.rotationYaw))) {
