...
public class RankupEvents {
    ...
    public static boolean canRankup(Account account, Rank currentRank, AutoRank rank) {
        ...
        // Check special cases
        if (rank.special_events != null && !rank.special_events.isEmpty() && !rank.special_events.equals("{}")) {
            RankupCondition[] conditions = null;
            try {
                conditions = ServerEssentials.GSON.fromJson(rank.special_events, RankupCondition[].class);
            } catch (Exception e) {
                try {
                    conditions = new RankupCondition[]{ServerEssentials.GSON.fromJson(rank.special_events, RankupCondition.class)};
                } catch (Exception f) {
    ...
    private static boolean checkRankupCondition(
        ...
        if (condition.type == null || condition.value == null)
            return false;
