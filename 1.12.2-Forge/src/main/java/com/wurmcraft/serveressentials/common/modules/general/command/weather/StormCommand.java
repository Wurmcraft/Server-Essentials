...
@ModuleCommand(module = "General", name = "Storm", defaultAliases = {"Lightning"})
public class StormCommand {
    ...
    public void storm(ServerPlayer player) {
        if (player.player == null)
            ...
            player.player.world.getWorldInfo().setRainTime(player.player.world.rand.nextInt(168000) + 12000);
            player.player.world.getWorldInfo().setThundering(true);
        }
        ChatHelper.send(player.sender, player.lang.COMMAND_WEATHER_STORM);
    }

    @Command(args = {CommandArgument.INTEGER}, usage = {"dimension"}, canConsoleUse = true)
