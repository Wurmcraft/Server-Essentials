...
public class TransferCommand {
    ...
    public static void send(ServerPlayer sender, String type) {
        try {
            ...
            } else if(type.equalsIgnoreCase("hotbar") || type.equalsIgnoreCase("hot") || type.equalsIgnoreCase("bar") || type.equalsIgnoreCase("hb")) {
                items = new ItemStack[9];
                for(int index = 0; index < 9; index++)
                    items[index] = pInv.getStackInSlot(index);
            } else if(type.equalsIgnoreCase("all") || type.equalsIgnoreCase("*") | type.equalsIgnoreCase("a")) {
                items = new ItemStack[36];
                for(int index = 0; index < 36; index++)
                    items[index] = pInv.getStackInSlot(index);
            }
            TransferEntry entry = new TransferEntry(-1,sender.player.getGameProfile().getId().toString(), Instant.now().getEpochSecond(), convert(items), TRANSFER_ID,false);
...
//            ChatHelper.send();
    ...
    public static ItemWrapper[] convert(ItemStack[] items) {
        return Arrays.stream(items).map(item -> new ItemWrapper(ServerEssentials.stackConverter.toString(item))).toArray(ItemWrapper[]::new);
    }
}
