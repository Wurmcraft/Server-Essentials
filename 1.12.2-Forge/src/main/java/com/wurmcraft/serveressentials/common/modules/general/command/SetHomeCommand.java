...
@ModuleCommand(module = "General", name = "SetHome", defaultAliases = {})
public class SetHomeCommand {
    ...
    public void setHome(ServerPlayer player, String name) {
        ...
        if (player.local.homes != null && player.local.homes.length < maxHomes || isOverride || player.local.homes == null) {
            Home newHome = new Home(player.player.posX, player.player.posY, player.player.posZ, player.player.dimension, player.player.rotationPitch, player.player.rotationYaw, name);
            // Add home to player
            if (isOverride) {
                ...
                player.local.homes = Arrays.copyOfRange(player.local.homes, 0, player.local.homes.length + 1);
            ...
            TextComponentString homeMSG = new TextComponentString(ChatHelper.replaceColor(player.lang.COMMAND_SETHOME.replaceAll("\\{@NAME@}", name)));
            homeMSG.setStyle(homeMSG.getStyle().setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponentString(
                    player.lang.HOME_OVER.replaceAll("\\{@X@}", Integer.toString((int) Math.round(newHome.x))).
                    ...
                    .setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "home " + newHome.name)));
            ChatHelper.send(player.sender, homeMSG);
        } else {
            ChatHelper.send(player.sender, player.lang.COMMAND_SETHOME_MAX.replaceAll("\\{@MAX@}", Integer.toString(maxHomes)));
        }
    }

    @Command(args = {CommandArgument.PLAYER, CommandArgument.STRING}, usage = {"player", "name"})
    ...
    @Command(args = {CommandArgument.STRING, CommandArgument.STRING}, usage = {"player", "name"})
    public void setHomeOffline(ServerPlayer player, String offlineUser, String name) {
        if (RankUtils.hasPermission(player.global, "command.home.other")) {
            ...
            if (offlineUUID != null) {
                int maxHomes = PlayerUtils.maxHomes(SECore.dataLoader.get(DataLoader.DataType.ACCOUNT, offlineUUID, new Account()));
                LocalAccount local = SECore.dataLoader.get(DataLoader.DataType.LOCAL_ACCOUNT, offlineUUID, new LocalAccount());
                if (local == null) {
                ...
                if (local.homes.length < maxHomes || isOverride) {
                    ...
                    // Send formatted message
                    TextComponentString homeMSG = new TextComponentString(player.lang.COMMAND_SETHOME_OTHER.replaceAll("\\{@NAME@}", name).replaceAll("\\{@PLAYER@}", Objects.requireNonNull(PlayerUtils.getUsernameForInput(offlineUUID))));
                    homeMSG.setStyle(homeMSG.getStyle().setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponentString(
                            player.lang.HOME_OVER.replaceAll("\\{@X@}", Integer.toString((int) Math.round(newHome.x))).
                                    replaceAll("\\{@Y@}", Integer.toString((int) Math.round(newHome.y))).
                                    replaceAll("\\{@Z@}", Integer.toString((int) Math.round(newHome.z))).
