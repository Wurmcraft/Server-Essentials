...
public class TPAAcceptCommand {
  ...
  public void acceptTPARequest(ServerPlayer player) {
    if (TPACommand.activeRequests.containsKey(player.player)) {
      ...
      ChatHelper.send(
          player.player,
          player.lang.COMMAND_TPACCEPT.replaceAll(
      ...
      ChatHelper.send(
          otherPlayer,
          otherLang.COMMAND_TPACCEPT_OTHER.replaceAll(
      ...
      ChatHelper.send(
          player.player,
          player.lang.COMMAND_TPACCEPT_HERE.replaceAll(
              "\\{@PLAYER@}", otherPlayer.getDisplayNameString()));
      Language otherLang =
          SECore.dataLoader.get(
              DataLoader.DataType.LANGUAGE,
      ...
      ChatHelper.send(
          player.player,
          otherLang.COMMAND_TPACCEPT_HERE_OTHER.replaceAll(
              "\\{@PLAYER@}", otherPlayer.getDisplayNameString()));
      if (TeleportUtils.teleportTo(
