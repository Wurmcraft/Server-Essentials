...
public class PayCommand {
  ...
  @Command(args = {CommandArgument.PLAYER, CommandArgument.DOUBLE}, usage = {"Player",
      "Amount"})
  public void pay(ServerPlayer player, EntityPlayer otherPlayer, double amount) {
    String serverCurrency = ((ConfigEconomy) SECore.moduleConfigs.get(
    ...
    if (EcoUtils.canBuy(serverCurrency, EcoUtils.computeTax(amount), account)) {
      ...
      ChatHelper.send(player.sender,
          player.lang.COMMAND_PAY.replaceAll("\\{@AMOUNT@}", "" + amount)
              .replaceAll("\\{@PLAYER@}", ChatHelper.getName(otherPlayer,
                  Objects.requireNonNull(PlayerUtils.getLatestAccount(
                      otherPlayer.getGameProfile().getId().toString())))));
      Language lang = CommandUtils.getPlayerLang(otherPlayer);
      ChatHelper.send(otherPlayer,
          lang.COMMAND_PAY_OTHER.replaceAll("\\{@AMOUNT@}", "" + amount)
