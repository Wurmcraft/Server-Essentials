...
@ModuleCommand(module = "General", name = "TpOffline", defaultAliases = {"OfflineTP", "OffTp", "OTp"})
public class TpOfflineCommand {
    ...
    public void tpOffline(ServerPlayer player, String user) {
        ...
        if (local != null) {
            if (TeleportUtils.teleportTo((EntityPlayerMP) player.player, player.local, local.lastLocation))
                ChatHelper.send(player.sender, player.lang.COMMAND_TPOFFLINE.replaceAll("\\{@PLAYER@}", UsernameCache.getLastKnownUsername(UUID.fromString(uuid))));
        } else
