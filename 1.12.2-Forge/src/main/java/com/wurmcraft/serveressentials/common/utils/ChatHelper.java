...
public class ChatHelper {
    ...
    public static void send(ICommandSender sender, String message) {
        TextComponentString text = new TextComponentString(replaceColor(message));
        if (((ConfigChat) SECore.moduleConfigs.get("CHAT")).displayUUIDOnHover) {
            TextComponentString hoverText = sender.getCommandSenderEntity() instanceof EntityPlayer ? new TextComponentString(((EntityPlayer) sender.getCommandSenderEntity()).getGameProfile().getId().toString()) : new TextComponentString("Console?");
