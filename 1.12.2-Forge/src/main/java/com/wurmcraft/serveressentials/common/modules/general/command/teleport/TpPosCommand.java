...
public class TpPosCommand {
  ...
  @Command(
      args = {CommandArgument.INTEGER, CommandArgument.INTEGER, CommandArgument.INTEGER},
      usage = {"x", "y", "z"})
  ...
  @Command(
      ...
      usage = {"player", "x", "y", "z", "dim"},
      canConsoleUse = true)
  public void teleportOtherPos(
      ServerPlayer player, EntityPlayer otherPlayer, int x, int y, int z, int dim) {
    if (RankUtils.hasPermission(player.global, "command.tppos.other")) {
      Location location =
          new Location(x, y, z, dim, otherPlayer.rotationPitch, otherPlayer.rotationYaw);
      if (TeleportUtils.teleportTo(
