...
public class CustomCommand extends CommandBase {
  ...
  public static final File SAVE_DIR =
      new File(ConfigLoader.SAVE_DIR + File.separator + "Commands" + File.separator + "Custom");
    ...
    if (sender instanceof EntityPlayer) {
      Account account =
          SECore.dataLoader.get(
              DataType.ACCOUNT,
              ((EntityPlayer) sender).getGameProfile().getId().toString(),
              new Account());
      // Min Rank Check
      if (command.minRank != null
          && !command.minRank.isEmpty()
      ...
      Language lang =
          (Language)
              SECore.dataLoader.get(
                  DataType.LANGUAGE, ((ConfigCore) SECore.moduleConfigs.get("CORE")).defaultLang);
  ...
  public static void createDefaultCommands() {
    CustomCommandJson websiteComamnd =
        new CustomCommandJson(
            "Website",
            new String[] {"Web", "Site"},
            new CommandFunc[] {
              new CommandFunc(
    ...
    try {
      Files.write(
          new File(SAVE_DIR + File.separator + "website.json").toPath(),
