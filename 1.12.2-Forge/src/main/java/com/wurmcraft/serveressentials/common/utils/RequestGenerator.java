...
public class RequestGenerator {
    ...
    // Regex
    public static final Pattern IP_REGEX = Pattern.compile("(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)");
    ...
    /**
     * Check and adds leading and trailing parts of a url if they are missing
     * This does not check if the url is valid or not, it simple makes sure it has the leading and trailing of a url
     *
    ...
    private static String parseConfigURL(String url) {
        ...
        if (!url.startsWith("http://") && !url.startsWith("https://"))
            url = IP_REGEX.matcher(url.substring(0, url.length() - 1)).find() ? "http://" + url : "https://" + url;
        return url;
    }
