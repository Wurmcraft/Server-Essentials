    ...
    if (PlayerUtils.isUserOnline(uuid)) {
      ...
      otherPlayer.connection.disconnect(
          new TextComponentString(
              ChatHelper.replaceColor(lang.COMMAND_DELETEPLAYERFILE_DISCONNECT)));
    }
    File playerFile =
        new File(
    ...
    if (playerFile.exists()) {
      ...
      try {
        ...
        if (playerFile.delete()) {
          ChatHelper.send(
              player.sender,
              player.lang.COMMAND_DELETEPLAYERFILE.replaceAll(
                  "\\{@PLAYER@}", "" + PlayerUtils.getUsernameForInput(playerName)));
        }
      } catch (Exception e) {
        e.printStackTrace();
      }
