...
public class BreakCommand {
    ...
    @Command(args = {}, usage = {}, isSubCommand = true, subCommandAliases = {"Connected", "Similar"})
    ...
    private static void destroyConnected(World world, Block block, BlockPos blockPos, int currentDepth) {
        if (currentDepth > 128) // Prevent deleting of the entire stone layer of the map (aka crash server)
            return;
        world.setTileEntity(blockPos, null);
        world.setBlockState(blockPos, Blocks.AIR.getDefaultState());
        if (!world.isAirBlock(blockPos.north()) && world.getBlockState(blockPos.north()).getBlock().getUnlocalizedName().equalsIgnoreCase(block.getUnlocalizedName()))
            destroyConnected(world, block, blockPos.north(), ++currentDepth);
        if (!world.isAirBlock(blockPos.south()) && world.getBlockState(blockPos.south()).getBlock().getUnlocalizedName().equalsIgnoreCase(block.getUnlocalizedName()))
            destroyConnected(world, block, blockPos.south(), ++currentDepth);
        if (!world.isAirBlock(blockPos.east()) && world.getBlockState(blockPos.east()).getBlock().getUnlocalizedName().equalsIgnoreCase(block.getUnlocalizedName()))
            destroyConnected(world, block, blockPos.east(), ++currentDepth);
        if (!world.isAirBlock(blockPos.west()) && world.getBlockState(blockPos.west()).getBlock().getUnlocalizedName().equalsIgnoreCase(block.getUnlocalizedName()))
            destroyConnected(world, block, blockPos.west(), ++currentDepth);
        if (!world.isAirBlock(blockPos.up()) && world.getBlockState(blockPos.up()).getBlock().getUnlocalizedName().equalsIgnoreCase(block.getUnlocalizedName()))
            destroyConnected(world, block, blockPos.up(), ++currentDepth);
        if (!world.isAirBlock(blockPos.down()) && world.getBlockState(blockPos.down()).getBlock().getUnlocalizedName().equalsIgnoreCase(block.getUnlocalizedName()))
            destroyConnected(world, block, blockPos.down(), ++currentDepth);
    }

    // TODO Implement
    @Command(args = {}, usage = {}, isSubCommand = true)
