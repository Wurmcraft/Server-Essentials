...
public class ReplyCommand {
    ...
    public void reply(ServerPlayer player, String msg) {
        if (ChatHelper.lastMessageCache.containsKey(player.player.getGameProfile().getId().toString())) {
            String lastMsg = ChatHelper.lastMessageCache.get(player.player.getGameProfile().getId().toString());
            ...
            if (otherPlayer != null)
                ...
                try {
                    SocketController.send(new WSWrapper(201, WSWrapper.Type.MESSAGE, new DataWrapper("DM", GSON.toJson(new DMMessage(player.player.getGameProfile().getId().toString(), ChatHelper.getName(player.player, player.global), ServerEssentials.config.general.serverID, msg, lastMsg)))));
