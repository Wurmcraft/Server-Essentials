...
public class PauseChatCommand {
  ...
  public void pauseAllChat(ServerPlayer player) {
    Channel[] channels =
        SECore.dataLoader
            .getFromKey(DataLoader.DataType.CHANNEL, new Channel())
            .values()
            .toArray(new Channel[0]);
    for (Channel ch : channels) pauseChannel(player, ch);
  }
  ...
  public void pauseChannel(ServerPlayer player, Channel ch) {
    if (ch.enabled) {
      ...
      ChatHelper.send(
          player.sender,
          player.lang.COMMAND_PAUSECHAT_DISABLED.replaceAll("\\{@CHANNEL@}", ch.name));
    } else {
      ch.enabled = true;
      SECore.dataLoader.update(DataLoader.DataType.CHANNEL, ch.name, ch);
      ChatHelper.send(
          player.sender,
          player.lang.COMMAND_PAUSECHAT_ENABLED.replaceAll("\\{@CHANNEL@}", ch.name));
    }
