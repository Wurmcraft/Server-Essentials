...
public class MarketListCommand {
  ...
  @Command(
      args = {CommandArgument.DOUBLE, CommandArgument.INTEGER},
      usage = {"Cost", "Count Per Listing"})
  public void listItem(ServerPlayer player, double cost, int perListing) {
    if (hasValidItem(player.player)) {
      ...
      if (maxListings
          < MarketHelper.countListings(player.player.getGameProfile().getId().toString(), false)) {
        ChatHelper.send(
            player.sender,
            player.lang.COMMAND_MARKETLIST_MAXED.replaceAll("\\{@COUNT@}", "" + maxListings));
        return;
      }
      ...
      if (listingsToCreate > 0) {
        ItemWrapper item =
            new ItemWrapper(ServerEssentials.stackConverter.toString(itemInQuestion));
        MarketEntry marketListing =
            new MarketEntry(
        ...
        ChatHelper.send(
            ...
            player
                .lang
                .COMMAND_MARKETLIST_LISTED
                .replaceAll("\\{@ITEM@}", ServerEssentials.stackConverter.toString(itemInQuestion))
        ...
        ChatHelper.send(
            player.sender,
            player.lang.COMMAND_MARKETLIST_INVALID_ITEM.replaceAll(
                "\\{@ITEM@}",
                ServerEssentials.stackConverter.toString(player.player.getHeldItemMainhand())));
      }
    } else {
      ChatHelper.send(
          player.sender,
  ...
  private static boolean hasValidItem(EntityPlayer player) {
    ...
    DataWrapper itemWrapper =
        ServerEssentials.stackConverter.getName(ServerEssentials.stackConverter.toString(heldItem));
    String itemFuzzy = "<" + itemWrapper.modid + ":" + itemWrapper.name + ">";
    String[] blackList = ((ConfigEconomy) SECore.moduleConfigs.get("ECONOMY")).itemBlacklist;
