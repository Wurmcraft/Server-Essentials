...
public class RestPlayerTrackerEvent {
    ...
    private static final NonBlockingHashMap<String, ScheduledFuture<?>> userSync = new NonBlockingHashMap<>();

    @SubscribeEvent
    public void onPlayerLogin(PlayerLoadEvent e) {
        if (!ServerEssentials.config.performance.useWebsocket)
            userSync.put(e.player.getGameProfile().getId().toString(), ServerEssentials.scheduledService.scheduleAtFixedRate(() -> {
                LOG.debug("Syncing User '" + e.player.getDisplayNameString() + "' (" + e.player.getGameProfile().getId().toString() + ")");
                if (SECore.dataLoader.delete(DataLoader.DataType.ACCOUNT, e.player.getGameProfile().getId().toString(), true))
                    SECore.dataLoader.get(DataLoader.DataType.ACCOUNT, e.player.getGameProfile().getId().toString());
            }, ServerEssentials.config.performance.playerSyncInterval, ServerEssentials.config.performance.playerSyncInterval, TimeUnit.SECONDS));
    }
