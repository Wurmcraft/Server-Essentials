...
public class WeatherCommand {
    ...
    @Command(args = {CommandArgument.STRING}, usage = "sun, rain, storm", canConsoleUse = true)
    public void weather(ServerPlayer player, String type) {
        if (type.equalsIgnoreCase("sun") || type.equalsIgnoreCase("sunny") || type.equalsIgnoreCase("su")) {
        ...
        } else if (type.equalsIgnoreCase("storm") || type.equalsIgnoreCase("st") || type.equalsIgnoreCase("lightning") || type.equalsIgnoreCase("light")) {
            for (WorldServer world : FMLCommonHandler.instance().getMinecraftServerInstance().worlds) {
                world.getWorldInfo().setRaining(true);
                world.getWorldInfo().setRainTime(world.rand.nextInt(168000) + 12000);
