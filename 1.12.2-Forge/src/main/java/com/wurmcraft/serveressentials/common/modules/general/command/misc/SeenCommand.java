...
@ModuleCommand(module = "General", name = "Seen", defaultAliases = {"LastSeen"})
public class SeenCommand {
    ...
    public void lookupLastSeen(ServerPlayer player, String otherPlayer) {
        ...
        if (uuid != null && !uuid.isEmpty() && account != null) {
            long lastSeen = 0;
            for (ServerTime time : account.trackedTime)
                if (time.lastSeen > lastSeen)
                    lastSeen = time.lastSeen;
            ChatHelper.send(player.sender, player.lang.COMMAND_SEEN.replaceAll("\\{@PLAYER@}", "" + PlayerUtils.getUsernameForInput(uuid)).replaceAll("\\{@TIME@}", CommandUtils.displayTime(lastSeen/ 1000)));
        } else
