...
public class ModuleGeneral {
    ...
    public void setup() {
        ...
        if (statusSchedule == null && SECore.dataLoader instanceof RestDataLoader)
            statusSchedule = ServerEssentials.scheduledService.scheduleAtFixedRate(() -> sendStatusUpdate(true, "Online"), ((ConfigGeneral) (SECore.moduleConfigs.get("GENERAL"))).statusSync, ((ConfigGeneral) (SECore.moduleConfigs.get("GENERAL"))).statusSync, TimeUnit.SECONDS);
    }
    ...
    public static void sendStatusUpdate(boolean useWebSocket, String status) {
        ...
        if (socket && useWebSocket) {
            try {
                SocketController.send(new WSWrapper(200, WSWrapper.Type.MESSAGE, new DataWrapper("Status", GSON.toJson(generateStatus(status)))));
            } catch (Exception e) {
