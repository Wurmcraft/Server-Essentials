...
public class NewChatPlayer {
    ...
    public void onNewPlayer(PlayerLoadEvent e) {
        ...
        if (SECore.dataLoader.getFromKey(DataLoader.DataType.BULLETIN, new Bulletin()) != null)
            for (Bulletin bulletin : SECore.dataLoader.getFromKey(DataLoader.DataType.BULLETIN, new Bulletin()).values()) {
                if (bulletin.expiration < System.currentTimeMillis()) {
                    ...
                    ServerEssentials.LOG.info("Bulletin '" + bulletin.title + "' has expired, deleting! (viewed by " + bulletin.viewedBy.size() + " players)");
                    continue;
                }
