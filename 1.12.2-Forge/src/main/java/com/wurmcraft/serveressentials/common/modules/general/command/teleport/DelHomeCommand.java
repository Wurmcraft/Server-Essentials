...
public class DelHomeCommand {
    ...
    public void delDefaultHome(ServerPlayer player) {
        delHome(player, PlayerUtils.getHome(player.local, ((ConfigGeneral) SECore.moduleConfigs.get("GENERAL")).defaultHomeName));
    }
    ...
    public void delHomeDefaultOther(ServerPlayer player, EntityPlayer otherPlayer) {
        delHomeDefaultOther(player, otherPlayer, ((ConfigGeneral) SECore.moduleConfigs.get("GENERAL")).defaultHomeName);
    }
        ...
        if (RankUtils.hasPermission(player.global, "command.delhome.other")) {
            ...
            if (offlineUUID != null) {
                ...
                if (local != null) {
                    ...
                    ChatHelper.send(player.sender, player.lang.COMMAND_DELHOME_OTHER.replaceAll("\\{@NAME@}", name).replaceAll("\\{@PLAYER@}", Objects.requireNonNull(PlayerUtils.getUsernameForInput(offlineUUID))));
                } else
