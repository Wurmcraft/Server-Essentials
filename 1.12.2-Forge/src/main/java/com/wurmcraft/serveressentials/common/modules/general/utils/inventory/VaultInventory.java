...
public class VaultInventory extends InventoryBasic {
    ...
    public VaultInventory(EntityPlayer player, Language lang, Vault vault, int page) {
        super(ChatHelper.replaceColor(lang.DISPLAY_VAULT.replaceAll("\\{@NAME@}", vault.name)) + " | " + page, true, 54);
        this.player = player;
        this.vault = vault;
        this.page = page;
        this.lang = lang;
    ...
    public void markDirty() {
        super.markDirty();
        try {
            File save = new File(ConfigLoader.SAVE_DIR + File.separator + FileDataLoader.SAVE_FOLDER + File.separator + "vaults" + File.separator + vault.ownerUUID + File.separator + vault.name + ".json");
            if (!save.exists()) {
                save.getParentFile().mkdirs();
                save.createNewFile();
            }
            Files.write(save.toPath(), GSON.toJson(vault).getBytes(), StandardOpenOption.WRITE, StandardOpenOption.TRUNCATE_EXISTING);
        } catch (Exception e) {
    ...
    private void handleAction(int index) {
        ...
        if (index == 3) {   // Options Menu
            ...
            player.closeScreen();
            player.displayGUIChest(new VaultInventory(player, lang, loadVault(vault.ownerUUID, vault.name), page));
            return;
        }
    ...
    private Vault loadVault(String ownerUUID, String name) {
        File save = new File(ConfigLoader.SAVE_DIR + File.separator + FileDataLoader.SAVE_FOLDER + File.separator + "vaults" + File.separator + ownerUUID + File.separator + name + ".json");
        if (save.exists()) {
            try {
