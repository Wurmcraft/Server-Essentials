...
@ModuleCommand(module = "General", name = "Freeze", defaultAliases = {"StopMoving",
    "Bubble"})
  ...
  public void freezePlayer(ServerPlayer sender, EntityPlayer player) {
    ...
    if (isFrozen) {
      ...
      ChatHelper.send(sender.sender,
          sender.lang.COMMAND_FREEZE_UNDO_OTHER.replaceAll("\\{@PLAYER@}",
      ...
      ChatHelper.send(player, SECore.dataLoader.get(DataType.LANGUAGE,
          ...
          new Language()).COMMAND_FREEZE_UNDO);
    } else {
      GeneralEvents.addFrozen(player,
          new BlockPos(player.posX, player.posY, player.posZ));
      ChatHelper.send(sender.sender,
          sender.lang.COMMAND_FREEZE_OTHER.replaceAll("\\{@PLAYER@}",
              ChatHelper.getName(player,
                  SECore.dataLoader.get(DataType.ACCOUNT,
                      player.getGameProfile().getId().toString(), new Account()))));
      ChatHelper.send(player, SECore.dataLoader.get(DataType.LANGUAGE,
