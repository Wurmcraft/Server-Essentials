...
public class ShopInventory extends InventoryBasic {
    ...
    private void handleAction(int index) {
        int maxPages = (entries.size() + 9) / 54;
        if (maxPages == 0)
            maxPages = 1;
    ...
    public ItemStack getStackInSlot(int index) {
        ...
        if (page > 1) {
            ...
            if (((index - 9) + ((54 - 9) * page)) < entries.size()) {
                return MarketHelper.getShopDisplayItem(entries.get((index - 9) + (page * (54 - 9))), lang, global);
            }
        }
    ...
    private void buyItem(MarketEntry entry) {
        ...
        if (!player.inventory.addItemStackToInventory(MarketHelper.getStackForMarketEntry(entry))) {
            ...
            ChatHelper.send(player,PlayerUtils.getLang(player).SENT_TO_MAILBOX);
        }
