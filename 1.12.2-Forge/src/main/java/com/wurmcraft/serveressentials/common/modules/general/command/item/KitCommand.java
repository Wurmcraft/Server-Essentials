...
public class KitCommand {
    ...
    public static String[] INVALID_KIT_NAMES = new String[]{"create", "c", "delete", "del", "d", "remove", "rem", "r", "list", "l'"};

    @Command(args = {CommandArgument.STRING}, usage = {"name"}, isSubCommand = true, subCommandAliases = {"c"})
    public void create(ServerPlayer player, String name) {
        for (String blacklist : INVALID_KIT_NAMES)
            if (name.equalsIgnoreCase(blacklist)) {
                ChatHelper.send(player.sender, player.lang.COMMAND_KIT_INVALID.replaceAll("\\{@NAME@}", name));
                return;
            }
        List<String> items = new ArrayList<>();
        String[] armor = new String[4];
        for (int index = 0; index < 4; index++)
            armor[index] = ItemStackConverter.toString(player.player.inventory.armorInventory.get(index));
        for (ItemStack item : player.player.inventory.mainInventory)
            items.add(ItemStackConverter.toString(item));
        Kit kit = new Kit(name, items.toArray(new String[0]), armor);
        SECore.dataLoader.register(DataLoader.DataType.KIT, kit.name, kit);
        ChatHelper.send(player.sender, player.lang.COMMAND_KIT_CREATED.replaceAll("\\{@NAME@}", kit.name));
    }
    ...
    public void list(ServerPlayer player) {
        ...
        ChatHelper.send(player.sender, player.lang.COMMAND_KIT_LIST.replaceAll("\\{@NAME@}", builder.toString()));
    }

    @Command(args = {CommandArgument.KIT}, usage = {"name"}, isSubCommand = true, subCommandAliases = {"del", "remove", "rem", "r", "d"}, canConsoleUse = true)
    public void delete(ServerPlayer player, Kit kit) {
        if (kit != null) {
            ...
            ChatHelper.send(player.sender, player.lang.COMMAND_KIT_DELETE.replaceAll("\\{@NAME@}", kit.name));
        }
    }

    @Command(args = {CommandArgument.KIT}, usage = {"name"})
            ...
            if (RankUtils.hasPermission(player.global, "command.kit." + kit.name)) {
                if ((player.local.kitUsage != null && player.local.kitUsage.containsKey(kit.name))) {
                    if (player.local.kitUsage.get(kit.name) < System.currentTimeMillis())
                        ...
                        ChatHelper.send(player.sender, player.lang.COMMAND_KIT_TIMER.replaceAll("\\{@TIME@}", CommandUtils.displayTime((player.local.kitUsage.get(kit.name) - System.currentTimeMillis()) / 1000)));
                } else
                    giveKit(player.player, kit, player.global, player.local);
            } else
    ...
    public static void giveKit(EntityPlayer player, Kit kit, Account global, LocalAccount account) {
        ...
        for (int index = 0; index < kit.items.length; index++) {
            ItemStack stack = ItemStackConverter.getData(kit.items[index]);
            if (stack.isEmpty())
                continue;
        ...
        for (int index = 0; index < kit.armor.length; index++) {
            ItemStack stack = ItemStackConverter.getData(kit.armor[index]);
            if (stack.isEmpty())
                continue;
        ...
        for (ItemStack stack : unsortedItems)
            if (!player.inventory.addItemStackToInventory(stack)) {
                player.world.spawnEntity(new EntityItem(player.world, player.posX, player.posY, player.posZ, stack)); // TODO Replace with placement into server overflow vault
            }
        // Update kit usage
        if(account.kitUsage == null)
            account.kitUsage = new HashMap<>();
        account.kitUsage.put(kit.name, System.currentTimeMillis() + CommandUtils.getTimeFromConfig(global.rank, kit.rankCooldown));
