...
public class HatCommand {
  ...
  public void placeHat(ServerPlayer player) {
    if (!player.player.inventory.armorInventory.get(3).isEmpty()) {
      player.player.inventory.addItemStackToInventory(
          player.player.inventory.armorInventory.get(3));
      player.player.inventory.armorInventory.set(3, ItemStack.EMPTY);
    }
    player.player.inventory.armorInventory.set(
        3, player.player.getHeldItemMainhand().splitStack(1));
