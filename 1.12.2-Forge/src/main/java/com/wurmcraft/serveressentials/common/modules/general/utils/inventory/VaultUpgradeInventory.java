...
public class VaultUpgradeInventory extends InventoryBasic {
  ...
  public VaultUpgradeInventory(EntityPlayer player, Language lang, Vault vault) {
    super(
        ChatHelper.replaceColor(lang.DISPLAY_VAULT.replaceAll("\\{@NAME@}", vault.name)), true, 54);
    this.player = player;
    this.lang = lang;
    this.vault = vault;
    menu = buildMenu();
  ...
  private ItemStack[] buildMenu() {
    ...
    info.setStackDisplayName(
        ChatHelper.replaceColor(PlayerUtils.getLang(player).DISPLAY_VAULT_ADDPAGE)
            + " "
            + calculateUpgradeCost(vault.maxPages));
    ...
    if (index == 22) {
      if (EcoUtils.canBuy(
        ...
        player.displayGUIChest(
            new VaultInventory(
                player, lang, VaultInventory.loadVault(vault.ownerUUID, vault.name), 0));
      } else {
  ...
  public static double calculateUpgradeCost(int maxPage) {
    return ((ConfigGeneral) SECore.moduleConfigs.get("GENERAL")).vaultPageBaseCost
        + (((ConfigGeneral) SECore.moduleConfigs.get("GENERAL")).vaultPageCostMultiplier * maxPage);
  }
}
