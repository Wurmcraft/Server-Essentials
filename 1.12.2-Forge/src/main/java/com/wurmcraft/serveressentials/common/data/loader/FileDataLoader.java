...
public class FileDataLoader extends DataLoader {
    ...
    public <T> NonBlockingHashMap<String, T> getFromKey(DataType key, T type) {
        ...
        if (saveDir.exists()) {
            ...
            if (saveDir.list() != null && Objects.requireNonNull(saveDir.list()).length == cachedData.size())
                return cachedData;
            // Load each file
    ...
    protected File getFile(DataType type, String key) {
        return new File(SAVE_DIR + File.separator + SAVE_FOLDER + File.separator + type.name().toLowerCase() + File.separator + key + ".json");
    }

    /**
     * Finds the requested data based on its key, if its cached its returned if not the file is attempted to be loaded
     *
        ...
        if (super.register(type, key, data)) {
            ...
            if (!file.exists()) {
                try {
                    ...
                    e.printStackTrace();
                    LOG.warn("Failed to write '" + file.getAbsolutePath() + "' for type '" + type.name() + "' key '" + key + "'");
                }
            } else
                LOG.debug("Failed to register '" + file.getAbsolutePath() + "' for type '" + type.name() + "' key '" + key + "'");
        }
        return false;
    }
    ...
    public boolean update(DataType type, String key, Object data) {
        if (super.update(type, key, data)) {
            ...
            if (file.exists()) {
                ...
                LOG.debug("Failed to update '" + file.getAbsolutePath() + "' for type '" + type.name() + "' key '" + key + "'");
        }
        return false;
    }
    ...
    public boolean delete(DataType type, String key, boolean cacheOnly) {
        ...
        if (file.exists() && file.delete()) {
            LOG.debug("Deleted file '" + file.getAbsolutePath() + "' type '" + type + "' key '" + key + "'");
            return true;
        }
        return false;
    }
