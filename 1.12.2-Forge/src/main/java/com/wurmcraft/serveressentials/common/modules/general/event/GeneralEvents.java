...
import com.wurmcraft.serveressentials.common.modules.general.command.misc.RulesCommand;
...
public class GeneralEvents {
    ...
    public void playerLoginEvent(PlayerEvent.PlayerLoggedInEvent e) {
        ...
        if (frozenPlayers == null || frozenPlayers.isEmpty())
            loadFreezeFile();
    }
    ...
    public void onPlayerMove(TickEvent.PlayerTickEvent e) {
        ...
        if (!frozenPlayers.isEmpty() && frozenPlayers.containsKey(e.player.getGameProfile().getId().toString())) {
            ...
            if (e.player.getPosition().getX() != lockedPos.x || e.player.getPosition().getY() != lockedPos.y || e.player.getPosition().getZ() != lockedPos.z) {
                e.player.setPositionAndRotation(lockedPos.x, lockedPos.y, lockedPos.z, (float) lockedPos.yaw, (float) lockedPos.pitch);
                e.player.setPositionAndUpdate(lockedPos.x, lockedPos.y, lockedPos.z); // Gotta do update, as cannot access directly
            }
    ...
    public static void removeFrozen(EntityPlayer player) {
        if (!frozenPlayers.isEmpty() && frozenPlayers.containsKey(player.getGameProfile().getId().toString())) {
    ...
    public static final File freezeFile = new File(ConfigLoader.SAVE_DIR + File.separator + "Storage" + File.separator + "freeze.json");
    ...
    public static void updateFreezeStorage() {
        ...
        if (freezeFile.exists()) {
            try {
                Files.write(freezeFile.toPath(), ServerEssentials.GSON.toJson(entries).getBytes(), StandardOpenOption.TRUNCATE_EXISTING);
            } catch (Exception e) {
            ...
            try {
                Files.write(freezeFile.toPath(), ServerEssentials.GSON.toJson(entries).getBytes(), StandardOpenOption.CREATE_NEW);
            } catch (Exception e) {
                ServerEssentials.LOG.error("Failed to write to freeze.txt");
                ServerEssentials.LOG.error(e.getMessage());
            }
    ...
    public static void loadFreezeFile() {
        try {
            if (freezeFile.exists()) {
                ...
                frozenPlayers = ServerEssentials.GSON.fromJson(Strings.join(frozen, "\n"), new TypeToken<HashMap<String, Location>>() {
                }.getType());
