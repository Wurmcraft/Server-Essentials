...
@ModuleCommand(
    name = "se",
    module = "Core",
    defaultAliases = {"Server-Essentials", "ServerEssentials"},
    defaultCooldown = {"*;5"})
public class BaseCommand {
  ...
  @Command(
      args = {CommandArgument.STRING, CommandArgument.MODULE},
      usage = {"reload, info", "module"},
      canConsoleUse = true)
  ...
  private void reloadModule(ServerPlayer player, String module, String m) {
    ...
    try {
      ...
      ChatHelper.send(
          player.sender,
          player.lang.COMMAND_BASE_RELOAD.replaceAll("\\{@MODULE@}", m.toUpperCase()));
      return;
    } catch (Exception e) {
      e.printStackTrace();
