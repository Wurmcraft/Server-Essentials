...
public class TransferRoutes {
    ...
    @OpenApi(
            ...
            tags = {"Transfer"},
            requestBody = @OpenApiRequestBody(content = {@OpenApiContent(from = TransferEntry.class)}, required = true, description = "Entry Entry information used to create the requested entry"),
    ...
    public static Handler getID = ctx -> {
        try {
            ...
            if (id < 0) {
                ctx.status(400).result(response("Invalid ID Path", "ID must be a number greater or equal to 0"));
                return;
            }
            ...
            if (entry != null) {
                ...
                ctx.status(404).result(response("Not Found", "Transfer ID with the provided ID does not exist"));
        ...
        try {
            ...
            if (isValidTransferEntry(ctx, updateEntry)) {
                try {
                    ...
                    ctx.status(400).result(response("Invalid Path", "Path ID must be a number greater or equal to 0"));
                }
            }
        } catch (JsonParseException e) {
            ctx.status(400).result(response("Invalid JSON", "Cannot Parse body into Transfer Entry"));
    ...
    public static Handler delete = ctx -> {
        try {
            ...
            if (entry != null) {
                ...
                ctx.status(404).result(response("Not Found", "Transfer Entry with the provided ID does not exist"));
        } catch (NumberFormatException e) {
            ctx.status(400).result(response("Invalid Path", "Path ID must be a number greater or equal to 0"));
        }
    };
