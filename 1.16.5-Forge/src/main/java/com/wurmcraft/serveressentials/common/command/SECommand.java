...
//            new NonBlockingHashSet<DelayedCommand>();
    ...
    public SECommand(CommandConfig config)
...
//        usage = usageArr.toArray(new String[0]);
    ...
    public void register(CommandDispatcher<CommandSource> commandDispatcher) {
        LiteralArgumentBuilder<CommandSource> commandBuilding = Commands.literal("serveressentials").requires(((user) -> hasPermission(ServerLifecycleHooks.getCurrentServer(), user.getEntity())));
        LiteralArgumentBuilder<CommandSource> argBuilder = commandBuilding.build().createBuilder();
        int count = 0;
        for (CommandArgument arg : (CommandArgument[]) arguments.keySet().toArray()) {
            argBuilder.then(Commands.argument("" + count, convert(arg)));
            count++;
        }
        commandBuilding = argBuilder.executes(this::commandExec);
        commandDispatcher.register(commandBuilding);
    }


    private int commandExec(CommandContext<CommandSource> cctx) {
        String arg0 = getArgFromIndex(cctx, 0).toString();
        cctx.getSource().sendSuccess(new StringTextComponent(arg0), true);
        return 0;
    }

    public static ArgumentType convert(CommandArgument arg) {
        return StringArgumentType.string();
    }

    public Object getArgFromIndex(CommandContext<CommandSource> cctx, int index) {
//        StringArgumentType arg = (StringArgumentType) cctx.getArgument(index + "", SECommand.convert((CommandArgument) this.arguments.keySet().toArray()[index]).getClass());
        return StringArgumentType.getString(cctx, index  + "");
    }
...
//    }
    ...
    public void runCommand(
            ServerPlayer userData, CommandSource sender, String[] args, CommandConfig config) {
        // Run Command
...
//                    EcoUtils.buy(userData.global, name, config.currencyCost.get(name));
            ...
            if (userData.sender instanceof PlayerEntity) {
...
//            ChatHelper.send(sender, getUsage(sender));
    ...
    private static int execCommand(CommandContext<CommandSource> commandSourceCommandContext) {
//        String arg = ServerEssentialsCommand.CommandArgument.get(commandSourceCommandContext, "arg");
...
//        }
        for (PlayerEntity player : ServerLifecycleHooks.getCurrentServer().getPlayerList().getPlayers()) {
            ...
            if (player.getDisplayName().getString().equalsIgnoreCase(name)) {
//                    || name.equalsIgnoreCase(ChatHelper.getName(player, account))) {
    ...
    public List<String> generateAliases() {
    ...
    public boolean hasPermission(MinecraftServer server, ICommandSource sender) {
...
//            MinecraftServer server, ICommandSource sender, String[] args, @Nullable BlockPos targetPos) {
//        int currentIndex = 0;
//        CommandArgument arg;
//        if (!args[0].isEmpty()) {
//            for (int index = 0; index < args.length; index++) {
