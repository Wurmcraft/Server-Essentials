...
public class FileWatcher {
  ...
  public FileWatcher(WatchService service, File trackedDirectory) throws IOException {
    ...
    ServerEssentialsBackend.scheduledService.scheduleAtFixedRate(
        handleFileChangeEvent(),
        0,
        ServerEssentialsBackend.backendConfiguration != null
            ? ServerEssentialsBackend.backendConfiguration.General.fileResyncInterval
            : 30,
        TimeUnit.SECONDS);
  }
   ...
   */
  public void track(
      TrackedFile eventRunnable, String name, boolean ignoreCase, BackupMode backupMode) {
  ...
  /**
   * IGNORE = Do nothing and ignore the update requests DELETE_ON_COMPLETE = Upon finishing
   * reloading without errors the backup will be deleted BACKUP_FOLDER = Move backup files to a
   * seperated nba
   */
