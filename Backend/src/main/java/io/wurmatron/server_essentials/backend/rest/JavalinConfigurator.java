...
//      cfg.enforceSsl = !config.general.testing;
      ...
      if (config.Server.swaggerEnabled) {
        cfg.registerPlugin(new OpenApiPlugin(new OpenApiOptions(
            new Info().version(ServerEssentialsBackend.VERSION)
                .description("Server Essentials Rest API")).path("api/swagger")
            .swagger(new SwaggerOptions("/swagger").title("Server-Essentials Swagger"))
            .roles(Route.RestRoles.values())));
        ServerEssentialsBackend.LOG.info(
            "Connect to swagger http://" + config.Server.host + ":" + config.Server.port
                + "/swagger");
      }
      cfg.requestLogger((ctx, ms) -> {
        ServerEssentialsBackend.LOG.debug(
            ctx.ip() + " " + ctx.method() + " " + ctx.path() + " (" + Math.round(ms)
                + "ms)");
      });
      cfg.accessManager((handler, ctx1, routeRoles) -> handler.handle(
