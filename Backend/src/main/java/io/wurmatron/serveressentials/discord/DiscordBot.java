...
public class DiscordBot {
        ...
        for (String serverID : ServerEssentialsRest.config.discord.channelMap.keySet()) {
            ...
            for (String s : split) {
                ...
                if (channelData.length == 2)
                    ...
                    LOG.warn("Invalid Channel map '" + serverID + "' must be in the format channelName:discordChannelID not '" + String.join(":", channelData) + "'");
            }
    ...
    public static void sendMessage(ChatMessage message) {
        ...
        if (channelMap.containsKey(message.serverID) && channelMap.get(message.serverID).containsKey(message.channel)) {
