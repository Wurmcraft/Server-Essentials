...
public class BotCommands {
  ...
  public static InteractionApplicationCommandCallbackReplyMono verify(ChatInputInteractionEvent e) {
    String username =
        e.getOption("username")
    ...
    if (e.getInteraction().getMember().isPresent()) {
      ...
      ServerEssentialsRest.executors.schedule(
          ...
          TimeUnit.SECONDS);
      return e.reply("Type `/verify " + verifyCode + "` in-game to verify").withEphemeral(true);
    }
    return e.reply(
