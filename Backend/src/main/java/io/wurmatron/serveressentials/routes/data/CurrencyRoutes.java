...
public class CurrencyRoutes {
      ...
      ctx -> {
        try {
          ...
          if (currency != null)
            ...
            ctx.status(404)
                .result(response("Not Found", "Currency with the provided ID does not exist"));
        } catch (NumberFormatException e) {
          ctx.status(400)
              .result(response("Bad Request", "ID must be a number, greater or equal to 0"));
        }
      };
  ...
  public static Handler override =
      ctx -> {
        ...
        if (currency != null) {
          ...
          ctx.status(404)
              .result(response("Not Found", "Currency with the provided Name does not exist"));
      };
  ...
  private static Currency filterBasedOnPerms(Context ctx, Currency currency) {
    ...
    if (role.equals(Route.RestRoles.DEV) || role.equals(Route.RestRoles.SERVER)) return currency;
  ...
  private static String createSQLForCurrencyWithFilters(Context ctx) {
    ...
    if (tax != null && !tax.trim().isEmpty()) sqlBuilder.append("tax=").append(tax).append(" AND ");
    // Finalize SQL
    sqlBuilder.append(";");
