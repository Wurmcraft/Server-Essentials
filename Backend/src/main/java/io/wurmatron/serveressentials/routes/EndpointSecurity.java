...
public class EndpointSecurity {
  ...
  public static Route.RestRoles getRole(Context ctx) {
    if (ServerEssentialsRest.config.general.testing) return Route.RestRoles.DEV;
    ...
    if (auth != null && authTokens.contains(auth)) {
      ...
      if (user.type.equalsIgnoreCase("USER")) return Route.RestRoles.USER;
      if (user.type.equalsIgnoreCase("SERVER")) return Route.RestRoles.SERVER;
    }
  ...
  public static void addSecurityManaging(Javalin app) {
    ...
    //            else
    //
    // ctx.contentType("application/json").status(401).result(response("Unauthorized", "You dont
  ...
  @Route(
      path = "api/login",
      method = "POST",
      roles = {Route.RestRoles.ANONYMOUS, Route.RestRoles.DEV})
        ...
        try {
          ...
          if (validateLogin(ctx, authUser)) {
            if (authUser.type.equalsIgnoreCase("USER")) {
              ...
              if (account != null) {
                if (EncryptionUtils.isSame(
                    account.password_hash, account.password_salt, authUser.token)) {
                  // User will now login
              ...
              if (authUser.token != null
                  && !authUser.token.isEmpty()
                  && authUser.key != null
                  && !authUser.key.isEmpty()
                  && serverAuth.containsKey(authUser.name)
                  && ctx.ip().equalsIgnoreCase(serverAuth.get(authUser.name).ip)) {
                ServerAuth auth = serverAuth.get(authUser.name);
                if (authUser.key.equals(auth.key)
                    && authUser.token.equals(auth.token)
                    && authUser.name.equals(auth.server_id)) {
                  // Server will login
  ...
  private static boolean validateLogin(Context ctx, AuthUser user) {
    // Validate login type
    if (user.type == null
        || !user.type.equalsIgnoreCase("USER") && !user.type.equalsIgnoreCase("SERVER")) {
  ...
  public static boolean hasPerms(Context ctx, String perm) {
    ...
    if (role.equals(Route.RestRoles.USER)) {
      ...
      return false;
    } else if (role.equals(Route.RestRoles.SERVER) || role.equals(Route.RestRoles.DEV)) return true;
    else return false;
  }
  ...
  public static String getPermNode(String path, String method) {
    if (permCache.contains(path + "_" + method.toLowerCase())) {
      ...
      if (split[0].equalsIgnoreCase("api")) startX = 1;
      for (int x = startX; x < split.length; x++) {
        if (split[x].startsWith(":")) break;
