...
public class LoggingRoutes {
  ...
  private static String createSQLForLogEntryWithFilters(Context ctx) {
    ...
    if (x != null && !x.trim().isEmpty()) sqlBuilder.append("x=").append(x).append(" AND ");
    // Verify, Check and Apply y Filter
    String y = ctx.queryParam("y");
    if (y != null && !y.trim().isEmpty()) sqlBuilder.append("y=").append(y).append(" AND ");
    // Verify, Check and Apply z Filter
    String z = ctx.queryParam("z");
    if (z != null && !z.trim().isEmpty()) sqlBuilder.append("z=").append(z).append(" AND ");
    // Verify, Check and Apply dim Filter
    String dim = ctx.queryParam("dim");
    if (dim != null && !dim.trim().isEmpty()) sqlBuilder.append("dim=").append(dim).append(" AND ");
    // Finalize SQL
    sqlBuilder.append(";");
