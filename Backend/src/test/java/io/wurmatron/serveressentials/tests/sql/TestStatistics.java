...
public class TestStatistics {
  ...
  public static final TrackedStat TEST_STAT =
      new TrackedStat(
          "Test", TestAccounts.TEST_ACCOUNT.uuid, Instant.now().getEpochSecond(), "break", "{}");
  ...
  public void testAddStat() {
    ...
    boolean found = false;
    for (TrackedStat ts : userStats) if (ts.equals(TEST_STAT)) found = true;
  ...
  public void testGetStat() {
    ...
    boolean found = false;
    for (TrackedStat ts : stats) if (ts.equals(TEST_STAT)) found = true;
  ...
  public void testDeleteStatEntry() {
    boolean deleted =
        SQLStatistics.delete(TEST_STAT.server_id, TEST_STAT.uuid, TEST_STAT.event_type);
