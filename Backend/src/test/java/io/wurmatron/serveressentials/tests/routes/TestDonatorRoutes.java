...
public class TestDonatorRoutes {
  ...
  public void testAddDonationEntry() throws IOException {
    Donator newEntry =
        HTTPRequests.postWithReturn("api/donator", TestDonators.TEST_DONATOR, Donator.class);
    assertNotNull(newEntry, "Entry was created successfully");
    // Make sure the donation entry was created
    Donator[] entries =
        HTTPRequests.get(
            "api/donator?transaction=" + TestDonators.TEST_DONATOR.transaction_id, Donator[].class);
  ...
  public void testGetDonationEntry() throws IOException {
    Donator[] entries =
        HTTPRequests.get("api/donator?uuid=" + TestDonators.TEST_DONATOR.uuid, Donator[].class);
    boolean exists = false;
    for (Donator entry : entries)
      if (TestDonators.TEST_DONATOR.equals(entry)) {
        exists = true;
  ...
  public void testUpdateDonationEntry() throws IOException {
    ...
    Donator[] entries =
        HTTPRequests.get("api/donator?type=" + TestDonators.TEST_DONATOR.type, Donator[].class);
    boolean exists = false;
    for (Donator entry : entries)
      if (TestDonators.TEST_DONATOR.equals(entry)) {
        exists = true;
  ...
  public void testDeleteDonationEntry() throws IOException {
    Donator deletedEntry =
        HTTPRequests.deleteWithReturn("api/donator", TestDonators.TEST_DONATOR, Donator.class);
