...
public class TestActionRoutes {
  ...
  public void testAddAction() throws IOException {
    Action addedAction =
        HTTPRequests.postWithReturn("api/action", TestActions.TEST_ACTION, Action.class);
  ...
  public void testGetAction() throws IOException {
    Action[] actions =
        HTTPRequests.get("api/action?host=" + TestActions.TEST_ACTION.host, Action[].class);
    boolean exists = false;
    for (Action action : actions)
      if (TestActions.TEST_ACTION.equals(action)) {
        exists = true;
  ...
  public void testDeleteAction() throws IOException {
    Action deletedAction =
        HTTPRequests.deleteWithReturn("api/action", TestActions.TEST_ACTION, Action.class);
    assertNotNull(deletedAction, "Action was been deleted successfully");
    // Make sure the action was deleted
    Action[] actions =
        HTTPRequests.get("api/action?action=" + TestActions.TEST_ACTION.action, Action[].class);
    boolean exists = false;
    for (Action action : actions)
      if (TestActions.TEST_ACTION.equals(action)) {
        exists = true;
